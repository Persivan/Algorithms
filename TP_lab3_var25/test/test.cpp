...
private:
   ...
   void rMagicFunc(Item*);                            //Добавление элемента по заданию
public:
   //Конструкторы
   /*
   ...
   */
   ...
   /*
   Ко всем вершинам дерева, которые являются листьями,
   добавить по одной дочерней вершине-листу; при этом
   к исходной вершине с нечетным значением добавляется
   левая дочерняя вершина, а к вершине с четным значением — правая.
   Значение каждой добавленной вершины положить равным значению ее родительской вершины.
   */
   void magicFunc();          //Функция по заданию
};
...
void Tree::rPrint(const string& prefix, Item* rot, bool isLeft, bool notAlone)
{
   if (rot)
   {
      cout << prefix << ((isLeft && notAlone) ? "|--" : "`--") << ((isLeft) ? "lf-" : "rg-") << rot->data << endl;
...
void Tree::rMagicFunc(Item* rot)
{
   //Пустой ли текущий элемент
   if (rot) 
   {
      //Является ли листом
      if (rot->isLeaf())
      {
         ...
         if (rot->data % 2 == 0)
         {
            rot->right = create(rot->data);
         }
         else
         {
            rot->left = create(rot->data);
         }
         //Еще четность можно проверить битовой операцией
         //if (rot->data & 1) {/*Выполнится если rot->data нечетный*/}
         //А сам if использовать тернарный или как он там называется
         //(rot->data & 1) ? rot->right = create(rot->data) : rot->left = create(rot->data);
      }
      else
      {
         //Рекурсивный переход к потомкам
         rMagicFunc(rot->left);
...
void Tree::print()
{
   ...
   {
      cout << "===Your tree===" << endl;
      cout << "lf - left branch, rg - right branch" << endl;
...
void Tree::magicFunc()
{
   ...
   {
      cout << "[magicFunc] Your tree is empty!" << endl;
   }
}
