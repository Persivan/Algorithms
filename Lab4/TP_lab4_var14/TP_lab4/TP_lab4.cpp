...
private:
   ...
   bool rCompare(Item*, Item*);                                   // Сравнить структуры
public:
   //Конструкторы
   /*
...
bool myNTree::rCompare(Item* rot1, Item* rot2)
{
   // Если оба пустые то возврращаем true
   if (!rot1 && !rot2)
   {
      return true;
   }
   // Если есть один но нет второго
   if ((!rot1 && rot2) || (rot1 && !rot2))
   {
      return false;
   }
   // Если есть оба проверяем дальше
   if (rot1 && rot2)
   {
      bool isSame = true;
      for (auto i = 0; i < N; i++)
      {
         isSame = isSame & rCompare(rot1->children[i], rot2->children[i]);
      }
      return isSame;
   }
}
...
void myNTree::magicFunc()
{
   if (root)
   {
      Item* secondTree = nullptr;
      int* simpleArray = nullptr;
      int size = fileIntArray("input2.txt", simpleArray);
      cout << "Array filled!" << endl;
      for (auto i = 0; i < size; i++)
      {
         // Добавление, скопированное из метода addElem
         if (secondTree)
         {
            int* lvls = new int[getHeight()];
            for (auto i = 0; i < getHeight(); i++)
            ...
            rCountElemsOnLvls(secondTree, lvls, 0);
            int* lvlsIsFull = new int[getHeight()];
            for (auto i = 0; i < getHeight(); i++)
            {
               if (lvls[i] == pow(N, i))
            ...
            bool flag = 0;
            rAddElem(secondTree, simpleArray[i], lvlToFill, 0, flag);
         }
         else
         {
            secondTree = new Item(simpleArray[i], N);
         }
      }
      // Вывод полученного дерева
      string myStrWithTree = "";
      rToString("   ", secondTree, true, 0, 0, myStrWithTree);
      cout << "   ===Second tree===\n" << myStrWithTree;
      // Проверка на совпадение по структуре
      cout << "Your second tree is the same? " << rCompare(root, secondTree) << endl;
   }
}
