...
int main()
{
   ...
   while (!list.isEmpty())
   {
      ...
      //Методом erase удаляем этот элемент.
      auto startCh = remove(strToModify.begin(), strToModify.end(), ch);
      strToModify.erase(startCh, strToModify.end());
   }
   cout << "After delete" << endl;
   cout << "string '" << strToModify << "'" << endl;
   cout << "list:" << endl;
   list.print();
   return 0;
}