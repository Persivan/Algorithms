...
int main()
{
   ...
   else
   {
      ...
      if (fin.is_open())
      {
         ...
         while (!fin.eof())
         {           
            //Мы берем по 2 строчки из файла, если в файле нечетное 
            //количество строк, то программа брала старые значения (то есть они повторялись)
   ...
   //Вывод на экран
   hashmap.print();
   //Поиск по ключу c защитой на пустые строки
   ...
   //Есть ли ключ-значение (без защиты от пустых строк)
...
Tree::Item* Tree::rAddElem(Item* rot, const string key, const string dt)
{
   if (rot)
   {
      if (rot->key.length() > key.length())
      {
         rot->left = rAddElem(rot->left, key, dt);
      }
      else if (rot->key.length() < key.length())
      {
