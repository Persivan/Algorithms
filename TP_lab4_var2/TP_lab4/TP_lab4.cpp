...
private:
   ...
   void rGetFirstLeafHeight(Item*, int&, int);                    // Получение высоты самого близкого к корню листа
   void rRemoveFirstLeafs(Item*, int, int);                       // Удалить самые дальние листья
public:
   //Конструкторы
   /*
   ...
   */
   ...
   void removeFirstLeafs();                  // Удаление по заданию
};
...
void myNTree::rGetFirstLeafHeight(Item* rot, int& height, int depth)
{
   // Выход если попали в пустой элемент
   if (!rot)
   {
      return;
   }
   if (rot->isLeaf() && depth < height)
   {
      height = depth;
   }
   ...
   for (int i = 0; i < N; i++)
   {
      rGetFirstLeafHeight(rot->children[i], height, depth);
   }
   depth--;
}

void myNTree::rRemoveFirstLeafs(Item* rot, int height, int depth)
{
   ...
   // Удаление элемента если он самый близкий
   for (int i = 0; i < N; i++)
   {
   ...
   for (int i = 0; i < N; i++)
   {
      rRemoveFirstLeafs(rot->children[i], height, depth);
   }
   depth--;
}
...
void myNTree::removeFirstLeafs()
{
   ...
   // Получаем высоту самых близких листьев
   int heightFirstLeaf = INT_MAX;
   rGetFirstLeafHeight(root, heightFirstLeaf, 1);
   // Удаление
   rRemoveFirstLeafs(root, heightFirstLeaf, 1);
}
