...
private:
   ...
   void rGetSumElems(Item*, int&);           //Посчитать сумму всех элементов
   void rPrintMagic(Item*);
   void rGetHeightWithOdd(Item*, int&);
public:
   //Конструкторы
   /*
   ...
   */
   ...
   const int getSumElems();                  //Получение суммы элементов
   void magic_func();
};
...
int main()
{
   ...
   //Вывод по заданию
   cout << "Sum of elems = " << newTree.getSumElems() << endl;
   newTree.magic_func();
   return 0;
}
...
void myNTree::rGetSumElems(Item* rot, int& sum)
{
   if (rot)
   {
      sum += rot->data;
      for (int i = 0; i < N; i++)
      {
         rGetSumElems(rot->children[i], sum);
      }
   }
}

void myNTree::rPrintMagic(Item* rot)
{
   //выход из рекурсии
   if (rot == nullptr)
   {
   ...
   //текущий узел четный? то выходим
   if ((rot->data & 1) == 0)
   {
      return;
   }
   cout << rot->data << " ";
   int* length = new int[N]; // Храним длины ветвей с нечетными вершинами
   //Переход на след уровень рекурсии
   for (int i = 0; i < N; i++)
   {
      rGetHeightWithOdd(rot->children[i], length[i]);
   }
   // Переход на след уровень по узлу у которого макс длина
   // Ищем в массиве длин максимальную
   int index = 0;
   for (int i = 0; i < N; i++)
   {
      if (length[index] < length[i])
      {
         index = i;
      }
   }
   //Спрашиваем
   rPrintMagic(rot->children[index]);
}

void myNTree::rGetHeightWithOdd(Item* rot, int& max)
{
   //выход из рекурсии
   if (rot == nullptr)
   {
   ...
   if (rot->data & 1)
   {
      ...
      for (int i = 0; i < N; i++)
      {
         rGetHeightWithOdd(rot->children[i], max);
      }
   }
}
...
const int myNTree::getSumElems()
{
   if (root)
   {
      int result = 0;
      rGetSumElems(root, result);
      return result;
   }
   else
   {
